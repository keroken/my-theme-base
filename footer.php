...
?>
            ...
            <div class="contact-container">
              ...
              <?php
              // Check if we're on a supporter page by looking for supporter-bg class in the body
              $body_classes = get_body_class();
              $is_supporter_page = false;

              // Alternative method: check if current template uses supporter header
              global $template;
              if (
                strpos($template, 'page-') !== false ||
                strpos($template, 'single-') !== false ||
                strpos($template, 'index.php') !== false
              ) {
                // Check if this page template calls get_header('supporter')
                $template_content = file_get_contents($template);
                if (
                  strpos($template_content, "get_header('supporter')") !== false
                ) {
                  $is_supporter_page = true;
                }
              }

              // Display Facebook link only on supporter pages
              if ($is_supporter_page): ?>
              <div class="contact-line">
                <svg class="contact-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512" fill="currentColor">
                  <path d="M80 299.3V512H196V299.3h86.5l18-97.8H196V166.9c0-51.7 20.3-71.5 72.7-71.5c16.3 0 29.2 .4 37 1.2V7.9C291.4 4 256.4 0 236.2 0C129.3 0 80 50.5 80 159.4v42.1H14v97.8H80z"/>
                </svg>
                <span><a href="https://www.facebook.com/ismc.ca" target="_blank" rel="noopener">Facebook</a></span>
              </div>
              <?php endif;
              ?>
            </div>
